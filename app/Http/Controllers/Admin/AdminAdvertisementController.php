...
class AdminAdvertisementController extends Controller
{
    ...
    public function update(Request $request) 
    {
        ...
        if($request->hasFile('job_listing_ad')) {
            ...
            unlink('uploads/'.$obj->job_listing_ad);

            $ext = $request->file('job_listing_ad')->extension();
            $final_name = 'job_listing_ad'.'.'.$ext;

            $request->file('job_listing_ad')->move('uploads/',$final_name);
        ...
        if($request->hasFile('company_listing_ad')) {
            ...
            unlink('uploads/'.$obj->company_listing_ad);

            $ext = $request->file('company_listing_ad')->extension();
            $final_name = 'company_listing_ad'.'.'.$ext;

            $request->file('company_listing_ad')->move('uploads/',$final_name);
        ...
        return redirect()->back()->with('success', 'Dữ liệu được cập nhật thành công.');
    }
}
