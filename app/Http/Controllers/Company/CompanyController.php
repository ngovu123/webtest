...
class CompanyController extends Controller
{
    ...
    public function vnpay(Request $request)
    {
        ...
        $vnp_TmnCode = "GEOXCI6H"; //Mã website tại VNPAY 
        $vnp_HashSecret = "YDIHDNNEDGBZNWSWAANHYEGPBOEKEWXF"; //Chuỗi bí mật
        $vnp_Url = " https://sandbox.vnpayment.vn/paymentv2/vpcpay.html";
        $vnp_Returnurl = "http://127.0.0.1:8000/company/make-payment";
    
        $vnp_TxnRef = $code_cart; //Mã đơn hàng. Trong thực tế Merchant cần insert đơn hàng vào DB và gửi mã này sang VNPAY
        $vnp_OrderInfo = "Thanh toán hóa đơn phí dịch vụ";
        $vnp_OrderType = 'billpayment';
        $vnp_Amount = $single_package_data->package_price * 100;
        $vnp_Locale = 'vn';
        $vnp_BankCode = 'NCB';
        ...
        $inputData = array(
            ...
            "vnp_CurrCode" => "VND",
            "vnp_IpAvnp_Locale" => $vnp_Locale,
            "vnp_OrderInfoddr" => $vnp_IpAddr,
    ...
    public function vnpay_success(Request $request)
    {
        $vnp_HashSecret = "YDIHDNNEDGBZNWSWAANHYEGPBOEKEWXF"; //Chuỗi bí mật
    ...
    if ($vnpSecureHash == $vnp_SecureHash) {
        // Xác thực thành công
        // Thực hiện các thao tác cần thiết khi thanh toán thành công
        
        $package_id = session()->get('package_id');
        $package_price = session()->get('package_price');
        $package_days = session()->get('package_days');
        ...
        $obj->order_no = time();
        $obj->paid_amount = $package_price;
        $obj->payment_method = 'Vnpay';
        $obj->start_date = date('Y-m-d');
        $days = $package_days;
        $obj->expire_date = date('Y-m-d', strtotime("+$days days"));
        $obj->currently_active = 1;
        ...
        return redirect()->route('company_make_payment')->with('error', 'Payment authentication failed!');
     ...
     return view('company.order', ['order' => $order]);
} 
    ...
    // $vnp_Returnurl = "http://localhost:8000/company/make-payment";
